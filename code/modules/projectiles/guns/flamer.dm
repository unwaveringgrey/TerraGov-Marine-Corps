...
/obj/item/weapon/gun/flamer/proc/flame_turf(turf/T, mob/living/user, heat, burn, f_color = "red")
	...
	for(var/mob/living/M in T) //Deal bonus damage if someone's caught directly in initial stream
		...
		if(isxeno(M))
			...
			if(istype(H.wear_suit, /obj/item/clothing/suit/fire) || (istype(H.wear_suit, /obj/item/clothing/suit/attachment/marine/M35) && istype(H.head, /obj/item/clothing/head/helmet/marine/pyro)))
				continue
...
/mob/living/carbon/human/flamer_fire_crossed(burnlevel, firelevel, fire_mod = 1)
	if(istype(wear_suit, /obj/item/clothing/suit/attachment/marine/M35) && istype(shoes, /obj/item/clothing/shoes/marine/pyro) && istype(head, /obj/item/clothing/head/helmet/marine/pyro))
		var/armor_block = run_armor_check(null, "fire")
...
/mob/living/carbon/human/flamer_fire_act(burnlevel, firelevel)
	if(istype(wear_suit, /obj/item/clothing/suit/fire) || istype(wear_suit,/obj/item/clothing/suit/space/rig/atmos) || (istype(wear_suit, /obj/item/clothing/suit/attachment/marine/M35) && istype(head, /obj/item/clothing/head/helmet/marine/pyro)))
